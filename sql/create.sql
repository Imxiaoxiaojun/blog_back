...
CREATE TABLE myblog.`tag` (
  `id`   BIGINT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `name` VARCHAR(45)         DEFAULT NULL,
  `color` VARCHAR(20) DEFAULT '#434343' COMMENT '标签颜色'
) ENGINE = InnoDB AUTO_INCREMENT = 50 DEFAULT CHARSET = utf8;
...
)ENGINE = InnoDB AUTO_INCREMENT = 50 DEFAULT CHARSET = utf8 COMMENT '评论';

DROP TABLE IF EXISTS myblog.`banner`;
CREATE TABLE myblog.`banner`(
  `id` BIGINT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY ,
  `banner_name` VARCHAR(50) NOT NULL ,
  `banner_intro` VARCHAR(200) DEFAULT NULL COMMENT '简介',
  `banner_url` VARCHAR(500) NOT NULL COMMENT '图片标题',
  `banner_type` VARCHAR(50) NOT NULL ,
  `creator` VARCHAR(50) DEFAULT NULL COMMENT '创建人',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后访问时间'
)ENGINE = InnoDB AUTO_INCREMENT = 50 DEFAULT CHARSET = utf8;
